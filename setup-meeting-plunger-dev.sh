...
main() {
  ...
  log "Starting meeting-plunger development environment setup"
  check_wsl2
  install_nixpkg_manager || rc=$?

  if [[ $rc -eq 0 ]]; then
    log "------------------------------------------ CONGRATS !!! ----------------------------------------------------"
    log "  meeting-plunger basic nix development environment tooling setup completed successfully."
    log "  Please exit this shell terminal and start a new one in meeting-plunger root directory then execute 'nix develop'."
